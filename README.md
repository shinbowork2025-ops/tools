# 業務補助ツール（静的ホスティング・PWA版）

静的なHTML・CSS・JavaScriptだけで動作するツール集です。HTTPS対応の静的ホスティングへ配置すると、Progressive Web App（PWA）として端末へインストールできます。Gitやビルド処理、外部パッケージは不要です。
...
node scripts/test-scan-consensus.mjs
```
...
- `scripts/test-scan-consensus.mjs`: JANの連続一致判定の自動検査
...
- 公開前に構文、PWA整合性、JAN連続一致判定の検査スクリプトを実行します。
