/**
 * 木材JANと材料マスタを連携する。
 * 未登録JANは現在の作業中だけ材料マスタへ追加し、アプリを閉じると破棄する。
 */
(() => {
  'use strict';
  ...
  scanRow.innerHTML = '<button id="woodScanButton" type="button" class="primary">JANをカメラで読む</button><p id="materialScanState" class="material-scan-state" aria-live="polite">登録済みJANは読み取り後すぐ図面へ追加します。未登録品は寸法入力後、現在の作業中だけ登録します。</p>';
  ...
  function selectByJan(rawValue) {
    ...
    setStatus(`JAN ${jan} は未登録です。商品名と寸法を入力し「手入力した材料を追加」を押してください。登録は現在の作業中だけ保持します。`);
  ...
  function storePendingMaterial() {
    ...
    const material = {
      id: `session-${pendingJan}-${Date.now().toString(36)}`,
      jan: pendingJan,
      name,
      width,
      height,
