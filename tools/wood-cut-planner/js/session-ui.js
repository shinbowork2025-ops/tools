(() => {
  ...
  const tabsWrap = document.querySelector('.sheet-tabs-wrap');
  const showAdd = document.getElementById('showAddSheetButton');
  if (tabsWrap && showAdd && newProject) {
    const actions = document.createElement('div');
    actions.className = 'sheet-actions';
    newProject.hidden = false;
    newProject.textContent = '作業をリセット';
    newProject.className = 'danger-text';
    actions.append(showAdd, newProject);
    tabsWrap.append(actions);
  }
  ...
  if (help) help.innerHTML = '<li>材料を選ぶと、その材料を追加して2D図を表示します。</li><li>図面の領域をタップし、辺と寸法を入力します。</li><li>選択中の材料タブの×で、その材料を削除できます。</li><li>削除やカット操作はUNDO・REDOで戻せます。</li>';
